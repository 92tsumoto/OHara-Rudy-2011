...
void function(double x[],double f[],double t)
{
	...
	f[4] = (ina.jss - x[4])/ina.tauj; // j
	f[5] = (ina.hCaMKss - x[5])/ina.tauh_CaMK_slow; // h_CaMK_slow
	f[6] = (ina.jCaMKss - x[6])/ina.tauj_CaMK; // j_CaMK
	//late sodium current
	...
	f[13] = (ito.aCaMKss - x[13])/ito.taua_CaMK;
	f[14] = (ito.iCaMKss - x[14])/ito.taui_CaMK_fast;
	f[15] = (ito.iCaMKss - x[15])/ito.taui_CaMK_slow;
	// LTCC
	...
	f[28] = (iks.xs2ss - x[28])/iks.tauxs2;
	// Ik1
...
{
	var.Ena = var.RTonF*log(var.nao/x[33]); // [Na]i = x[16],[Na]o = x[21]
	var.Ek = var.RTonF*log(var.ko/x[35]); // x[17] = [K]i, x[22] = [K]o
